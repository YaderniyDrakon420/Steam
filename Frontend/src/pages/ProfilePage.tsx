...
const API_BASE_URL = "https://localhost:7190/api/Profile/3";
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (!gRes.ok || !aRes.ok) throw new Error("API Error");
        ...
        setUser({
          ...
          level: 1,
          balance: 0.00,
          gamesCount: gData.length,
          achievementsCount: aData.filter((x: Achievement) => x.unlocked).length
        });
      } catch (err) {
  ...
  if (!user) return <div className="status-screen"><style>{css}</style>Error. Check Backend.</div>;
